from wtwu.packages.data import (
    create_global_dataset,
    create_batches,
    validate_patient_data,
    create_time_dependent_dataset
)
...
if __name__ == "__main__":
    ...
    prefix = "preprocessed"
    batch_size = 32
    patients = [str(i) for i in range(284, 384)] # Liste des patients (à ajuster selon vos données)

    print("Current working directory:", os.getcwd())

    # Vérification des arguments pour les fenêtres temporelles
    if len(sys.argv) > 1:
    ...
    # Étape 2 : Charger les données globales ou temporelles
    if len(sys.argv) > 1:
    ...
    # Étape 5 : Optimiser les hyperparamètres et créer le meilleur modèle
    print("Lancement de l'optimisation des hyperparamètres...")
    best_model, best_hps = optimize_model(X_train, y_train, X_val, y_val, max_trials=10)
    ...
    if len(sys.argv) > 1:
        model_path = save_model_local(best_model, time_window=init_time, window_size=window_size)
    else:
        model_path = save_model_local(best_model)
    ...
    # Sauvegarder les métriques dans BigQuery
    ...
    # Sauvegarde des meilleurs hyperparamètres localement
    from wtwu.packages.storage import save_best_hps_local_hyperparameters
    save_best_hps_local_hyperparameters(
        directory="./Metrics_wtwa/Hps",
        best_hps=best_hps,
        metrics={
            "accuracy": max(history.history['val_accuracy']),
            "loss": min(history.history['val_loss']),
            "recall": min(history.history['val_recall'])
        }
    )
    print("Sauvegarde des meilleurs hyperparamètres terminée.")
